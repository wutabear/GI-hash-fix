...
def simple_request(url: str, error_count: int = 0) -> Response:
    if not url.casefold().startswith("http"):
        raise urllib.error.URLError("malformed url")

    httprequest = urllib.request.Request(url, headers={"Accept": "application/json"})
    try:
    ...
    for ini_file in ini_files:
        try:
            ...
            with open(os.path.join(folder_path, ini_file), "r+", encoding="utf-8") as f:
                ...
                for match in matching:
                    if match in oldvsnew_41 and oldvsnew_41[match] not in data:
                        ...
                        with open(os.path.join(folder_path,
                                               "DISABLED_Backup_41" + os.path.splitext(ini_file)[0] + ".txt", ), "w",
                                  encoding="utf-8", ) as g:
                            g.write(data)
                        data += f"\n\n; 4.1 Character Fix \n[TextureOverride41FixVertexLimitRaise{counter}]\nhash = {oldvsnew_41[match]}\nmatch_priority = 1"
                        counter += 1
                    elif (match in DetermineCharacterBasedOnPositionHash_41 and
                          DetermineCharacterBasedOnPositionHash_41[match] not in data):
                        print("\tUpdating ini for 4.1")
                        with open(os.path.join(folder_path,
                                               "DISABLED_Backup_41" + os.path.splitext(ini_file)[0] + ".txt", ), "w",
                                  encoding="utf-8", ) as g:
                ...
                # 4.3 Fix
                matching = [x for x in old_hashes_ib_43 if str(x) in data] or [x for x in new_hashes_ib_43 if
                                                                               str(x) in data]
                ...
                for match in matching:
                    ...
                    if match in oldvsnew_43 and oldvsnew_43[match] not in data:
                        ...
                        with open(os.path.join(folder_path,
                                               "DISABLED_Backup_43" + os.path.splitext(ini_file)[0] + ".txt", ), "w",
                                  encoding="utf-8", ) as g:
                            g.write(data)
...
def test_simple_request():
    url = 'https://raw.githubusercontent.com/wutabear/genshinhashfix/main/alljson_43.json'
